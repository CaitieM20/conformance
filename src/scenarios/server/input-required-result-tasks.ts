...
export class InputRequiredResultTaskBasicScenario implements ClientScenario {
  name = 'input-required-result-task-basic';
  specVersions: SpecVersion[] = ['draft'];
  description = `Test full persistent InputRequiredResult workflow via Tasks API (SEP-2322).

**Server Implementation Requirements:**

Implement a tool named \`test_input_required_result_task\` that supports task-augmented execution.
...
3. When \`tasks/result\` is called, return an \`InputRequiredResult\` with \`inputRequests\`:
...
6. When \`tasks/result\` is called again, return the final result with tool content`;
  ...
  async run(serverUrl: string): Promise<ConformanceCheck[]> {
    ...
    try {
      ...
      checks.push({
        id: 'input-required-result-task-created',
        name: 'InputRequiredResultTaskCreated',
      ...
      checks.push({
        id: 'input-required-result-task-input-required',
        name: 'InputRequiredResultTaskInputRequired',
      ...
      } else if (!isInputRequiredResult(r3Result)) {
        r3Errors.push(
          'Expected InputRequiredResult with inputRequests from tasks/result'
        );
      } else if (!r3Result.inputRequests) {
        r3Errors.push(
          'InputRequiredResult from tasks/result missing inputRequests'
        );
      }
      ...
      checks.push({
        id: 'input-required-result-task-input-response-sent',
        name: 'InputRequiredResultTaskInputResponseSent',
      ...
      if (r4Errors.length === 0) {
        ...
        checks.push({
          id: 'input-required-result-task-ack-structure',
          name: 'InputRequiredResultTaskAckStructure',
          description:
      ...
      checks.push({
        id: 'input-required-result-task-completed',
        name: 'InputRequiredResultTaskCompleted',
      ...
      checks.push({
        id: 'input-required-result-task-final-result',
        name: 'InputRequiredResultTaskFinalResult',
...
export class InputRequiredResultTaskBadInputResponseScenario
  implements ClientScenario
{
  name = 'input-required-result-task-bad-input-response';
  specVersions: SpecVersion[] = ['draft'];
  description = `Test error handling when tasks/input_response contains wrong data (SEP-2322).

**Server Implementation Requirements:**

Use the same tool as B1: \`test_input_required_result_task\`.
...
**Behavior:** When the client sends \`tasks/input_response\` with incorrect keys, the server SHOULD acknowledge the message but keep the task in \`input_required\` status. The next \`tasks/result\` call should return a new \`inputRequests\` re-requesting the needed information.`;
  ...
  async run(serverUrl: string): Promise<ConformanceCheck[]> {
    ...
    try {
      ...
      if (!task?.taskId) {
        checks.push({
          id: 'input-required-result-task-bad-input-prereq',
          name: 'InputRequiredResultTaskBadInputPrereq',
      ...
      if (
        r3.error ||
        !r3.result ||
        !isInputRequiredResult(r3.result) ||
      ...
      if (stillInputRequired) {
        ...
        if (
          r5.result &&
          isInputRequiredResult(r5.result) &&
      ...
      checks.push({
        id: 'input-required-result-task-bad-input-rerequests',
        name: 'InputRequiredResultTaskBadInputRerequests',
        description:
...
// ─── B4: tasks/input_response returning InputRequiredResult ─────────────────────

export class InputRequiredResultTaskInputResponseInputRequiredScenario
  implements ClientScenario
{
  name = 'input-required-result-task-input-response-incomplete';
  specVersions: SpecVersion[] = ['draft'];
  description = `Test that tasks/input_response can itself return an InputRequiredResult (SEP-2322).

**Server Implementation Requirements:**

Implement a tool named \`test_input_required_result_task_multi_input\` that supports task-augmented execution and requires TWO rounds of input.

**Behavior:**
1. Create task, transition to \`input_required\`
2. \`tasks/result\` returns InputRequiredResult with first \`inputRequests\`
3. \`tasks/input_response\` returns an \`InputRequiredResult\` with ADDITIONAL \`inputRequests\`
4. Client sends another \`tasks/input_response\` with the additional responses
5. Task completes

This tests the schema: \`TaskInputResponseResultResponse.result: Result | InputRequiredResult\``;
  ...
  async run(serverUrl: string): Promise<ConformanceCheck[]> {
    ...
    try {
      ...
      if (!task?.taskId) {
        checks.push({
          id: 'input-required-result-task-multi-input-prereq',
          name: 'InputRequiredResultTaskMultiInputPrereq',
          description: 'Prerequisite: Task creation',
          status: 'FAILURE',
          timestamp: new Date().toISOString(),
          errorMessage: 'Could not create task',
      ...
      // Send first input_response — expect InputRequiredResult back
      ...
      } else if (!isInputRequiredResult(r4Result)) {
        r4Errors.push(
          'Expected InputRequiredResult from tasks/input_response (additional input needed)'
        );
      } else if (!r4Result.inputRequests) {
        r4Errors.push(
          'InputRequiredResult from tasks/input_response missing inputRequests'
        );
      }

      checks.push({
        id: 'input-required-result-task-input-response-returns-incomplete',
        name: 'InputRequiredResultTaskInputResponseReturnsIncomplete',
        description:
          'tasks/input_response returns InputRequiredResult with additional inputRequests',
        status: r4Errors.length === 0 ? 'SUCCESS' : 'FAILURE',
        timestamp: new Date().toISOString(),
        errorMessage: r4Errors.length > 0 ? r4Errors.join('; ') : undefined,
        specReferences: MRTR_SPEC_REFERENCES,
      ...
      if (r4Errors.length === 0 && isInputRequiredResult(r4Result)) {
        ...
        if (r5Ok) {
          ...
          checks.push({
            id: 'input-required-result-task-multi-input-completed',
            name: 'InputRequiredResultTaskMultiInputCompleted',
